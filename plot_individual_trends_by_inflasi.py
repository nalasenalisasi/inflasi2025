...
# Output directory
output_dir = 'grafik_komoditas_inflasi'
...
for display_name, db_name in target_map.items():
    ...
    plt.plot(data['Bulan'], data['Inflasi MtM'], marker='o', linestyle='-', linewidth=2, color='#d62728') # Red for Inflation
    
    # Add value labels
    for x, y in zip(data['Bulan'], data['Inflasi MtM']):
        label = f"{y:.2f}"
        plt.annotate(
            label,
            (x, y),
    ...
    plt.title(f'Tren Inflasi MtM - {display_name} (2025)', fontsize=14, pad=15)
    ...
    output_file = os.path.join(output_dir, f'{safe_name}_inflasi.png')
...
print("\nSelesai membuat semua grafik individu (basis Inflasi MtM).")
